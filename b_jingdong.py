...
def parsePage(List, html):
    try:
        plt = re.findall(r'<em>￥</em><i>.*?\.\d\d',html)#正则过滤价格
        tlt = re.findall(r'[^(<em>￥</em>)]<em>.*?[\u4e00-\u9fa5].*?</em>',html)#过滤标题
        for i in range(len(plt)):#遍历
               jd_price = plt[i].split('<i>')[1]
               jd_title = tlt[i]#切片
               List.append([jd_price,jd_title])
    except:
        print("")
...
def printGoodsList(List):
    ...
    # 写入文件
    f = open('京东价格清单.txt', 'w', encoding='utf-8')#utf-8写中文
    f.write('价格         商品名称\n')
    count = 0
    for g in List:
        count = count + 1
...
def jingdong():
    ...
    start_url = 'https://search.jd.com/Search?keyword='+ goods.get()+'&enc=utf-8&wq='+goods.get()
    infoList = []
...
laber_1 = tk.Label(root,text = "欢迎使用京东商品查询系统")
...
imgInfo = tk.PhotoImage(file = "Logo_jingdong.png")#图片插入
